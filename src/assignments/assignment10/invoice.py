...
class Invoice:
    ...
    def print_invoice(self):
        ...
        print('Check the outcome:')    
        print('Customer: ', self.customer.first, self.customer.last)

        total_cost = 0

        print('Product name', 'Quantity', '     Cost', '   Extended Cost')
        ...
        for invoice_item in self.invoice_items:
            ...
            print(invoice_item.get_description(), format(invoice_item.quantity, '12d'), \
                  format(invoice_item.product.cost, '14,.2f'), format(invoice_item.get_extended_cost(), '12,.2f'))
